        ...
        <div class="cartBox">
            ...
            <div class="order_content">
                <ul class="order_lists" v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="list_con">
                        <div class="list_img"><img :src="item.wenhuachanpinPhoto"></div>
                        <div class="list_text">{{item.wenhuachanpinName}}</div>
                    </li>
                    <li class="list_price">
                        <p class="price">{{item.wenhuachanpinNewMoney}} RMB</p>
                    </li>
                    ...
                    <li class="list_sum">
                        <p class="sum_price">{{(item.wenhuachanpinNewMoney*item.buyNumber).toFixed(2)}} RMB</p>
                    </li>
                    <li class="list_op">
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`wenhuachanpin/info/${item.wenhuachanpinId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.wenhuachanpinKucunNumber && res.data.wenhuachanpinKucunNumber > 0 && res.data.wenhuachanpinKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`文化产品库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('wenhuachanpins', JSON.stringify(this.dataList));
				window.location.href = '../wenhuachanpinOrder/confirm.jsp'
			}
        }
    });
