...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的文化产品订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部文化产品订单</li>
                    <li v-for="item in wenhuachanpinOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.wenhuachanpinName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.wenhuachanpinPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.wenhuachanpinValue}}</td>
                            <td style="width: 60px;">{{item.wenhuachanpinKucunNumber}}</td>
                            <td style="width: 100px;color: red">{{item.wenhuachanpinOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.wenhuachanpinNewMoney}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 60px;">{{item.wenhuachanpinClicknum}}</td>
                            <td style="width: 150px;">{{item.wenhuachanpinOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.wenhuachanpinOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.wenhuachanpinOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.wenhuachanpinOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.wenhuachanpinOrderValue}}</td>
                            <td style="width: 80px;">{{item.wenhuachanpinOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">


                                <button v-if="item.wenhuachanpinOrderTypes==3" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.wenhuachanpinOrderTypes==4" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(wenhuachanpinOrderTypes) {
                this.wenhuachanpinOrderTypes = wenhuachanpinOrderTypes;
                // 获取列表数据
                layui.http.request('wenhuachanpinOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    wenhuachanpinOrderTypes: (vue.wenhuachanpinOrderTypes == 0)?null:vue.wenhuachanpinOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('wenhuachanpinOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    wenhuachanpinOrderTypes: (this.wenhuachanpinOrderTypes == 0)?null:this.wenhuachanpinOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wenhuachanpinOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wenhuachanpinOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wenhuachanpinOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#wenhuachanpinCommentbackModal').modal('show');//打开模态框
            }
            ,submitWenhuachanpinCommentback(){
                var wenhuachanpinCommentbackContent = $("#wenhuachanpinCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("wenhuachanpinOrder/commentback?id="+id+"&commentbackText="+wenhuachanpinCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function wenhuachanpinOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenhuachanpin_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function wenhuachanpinOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenhuachanpin_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('wenhuachanpinOrder/page', 'get',{
            page: 1,
            limit: limit,
            wenhuachanpinOrderTypes: (vue.wenhuachanpinOrderTypes == 0)?null:vue.wenhuachanpinOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('wenhuachanpinOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            wenhuachanpinOrderType: (vue.wenhuachanpinOrderTypes == 0)?null:vue.wenhuachanpinOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
