        ...
        <div class="section-con">
            ...
            <div class="orderBox">
                <div class="orderC-table">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td class="text-left">
                                <div class="order-gw clearfix">
                                    <div class="order-img"><img :src="item.wenhuachanpinPhoto"/></div>
                                    <div class="order-sum">
                                        <h2>{{item.wenhuachanpinName}}</h2>
                                    </div>
                                </div>
                            </td>
                            <td>{{item.wenhuachanpinNewMoney}} RMB</td>
                            <td>
                                {{item.buyNumber}}
                            </td>
                            <td >
                                <span class="priceD">￥{{(item.wenhuachanpinNewMoney*item.buyNumber).toFixed(2)}}</span>
                            </td>
                        </tr>
                        </tbody>
                    </table>

                    <div class="orderC-total" v-if="this.wenhuachanpinOrderPaymentTypes == 1">
                        <div class="order-t">
                            <dl class="clearfix">
                    ...
                    <div class="orderC-total" v-if="this.wenhuachanpinOrderPaymentTypes == 2">
                        <div class="order-t">
                            <dl class="clearfix">
                                <dd class="dl_ddL">总&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</dd>
		...
		<script>
			var vue = new Vue({
				...
				data: {
					...
					dataList: [],
					addressList: [],
                    wenhuachanpinOrderPaymentTypes: 1,
                    zhekou:1,
					// 当前用户信息
					user: {}
				},
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.wenhuachanpinNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            ...
                            wenhuachanpins:localStorage.getItem('wenhuachanpins'),
                            yonghuId: localStorage.getItem('userid'),
                            wenhuachanpinOrderPaymentTypes: vue.wenhuachanpinOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`wenhuachanpinOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../wenhuachanpinOrder/list.jsp';
							}
                       		 window.location.href='../wenhuachanpinOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取文化产品购买的清单列表
				var wenhuachanpins = localStorage.getItem('wenhuachanpins');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(wenhuachanpins);
