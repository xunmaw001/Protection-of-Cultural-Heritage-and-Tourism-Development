...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的景点购票订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 100px;color: red">{{item.jingdianOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.jingdianNewMoney}}</td>
                            <td style="width: 60px;">{{item.jingdianClicknum}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 150px;">{{item.jingdianOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.jingdianOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.jingdianOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.goumaiTime}}</td>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            jingdianOrderPaymentTypesList : [],
            jingdianOrderTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            //列表查询
            ,pageList(jingdianOrderTypes) {
                this.jingdianOrderTypes = jingdianOrderTypes;
                // 获取列表数据
                layui.http.request('jingdianOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    jingdianOrderTypes: (vue.jingdianOrderTypes == 0)?null:vue.jingdianOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('jingdianOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    jingdianOrderTypes: (this.jingdianOrderTypes == 0)?null:this.jingdianOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`jingdianOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`jingdianOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`jingdianOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#jingdianCommentbackModal').modal('show');//打开模态框
            }
            ,submitJingdianCommentback(){
                var jingdianCommentbackContent = $("#jingdianCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("jingdianOrder/commentback?id="+id+"&commentbackText="+jingdianCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function jingdianOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdian_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jingdianOrderPaymentTypesList = res.data.list;
                }
            });
        }
        ...
        // 获取列表数据
        http.request('jingdianOrder/page', 'get',{
            page: 1,
            limit: limit,
            jingdianOrderTypes: (vue.jingdianOrderTypes == 0)?null:vue.jingdianOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('jingdianOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            jingdianOrderType: (vue.jingdianOrderTypes == 0)?null:vue.jingdianOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
