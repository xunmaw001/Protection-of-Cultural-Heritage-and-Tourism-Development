                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="wenhuachanpinName" id="wenhuachanpinName" placeholder="文化产品名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">文化产品类型</label>
                              <div class="layui-input-inline">
                                  <select name="wenhuachanpinTypes" id="wenhuachanpinTypes" lay-filter="wenhuachanpinTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in wenhuachanpinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteWenhuachanpinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../wenhuachanpin/detail.jsp?id='+item.wenhuachanpinId , item.wenhuachanpinDelete == null?'':item.wenhuachanpinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.wenhuachanpinPhoto?item.wenhuachanpinPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.wenhuachanpinName}}</h4>
                                        <p v-if="item.wenhuachanpinNewMoney != null" >￥{{item.wenhuachanpinNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            wenhuachanpinCollectionTypesList : [],
        // 级联表字典表数据
            wenhuachanpinTypesList : [],
            shangxiaTypesList : [],
            wenhuachanpinCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteWenhuachanpinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('wenhuachanpinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        wenhuachanpinCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function wenhuachanpinCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenhuachanpin_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wenhuachanpinCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function wenhuachanpinTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenhuachanpin_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wenhuachanpinTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#wenhuachanpinName').val() != null && jquery('#wenhuachanpinName').val() != "" && jquery('#wenhuachanpinName').val() != "null" ) {
                param['wenhuachanpinName'] = jquery('#wenhuachanpinName').val();
            }
                                         
            var wenhuachanpinTypes = $("#wenhuachanpinTypes").find("option:selected").val();
            if (wenhuachanpinTypes != null && wenhuachanpinTypes != "" && wenhuachanpinTypes != "null") {
                            param['wenhuachanpinTypes'] = wenhuachanpinTypes;
                        }
                                                                                                                                                                
            // 当前表
                                                
            //收藏只能查类型为1的
            param['wenhuachanpinCollectionTypes'] = 1;

            // 获取列表数据
            http.request('wenhuachanpinCollection/page', 'get', param, function(res) {
